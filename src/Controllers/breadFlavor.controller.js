...
export const deletep = async (req,res) =>{
    try
    {
        const [resultado] = await pool.query("DELETE FROM sabor_pan WHERE Id = ?",[req.params.id])
        if(resultado.affectedRows <= 0)return res.status(404).json({message: "sabor de pan no encontrado "})
...
export const update = async (req,res) =>{
    try
    {
        const {id} = req.params
        const {Nombre,Precio} = req.body;
        const [resultado] = await pool.query("UPDATE sabor_pan SET Nombre= IFNULL(?,Nombre), Precio = IFNULL(?, Precio) WHERE Id = ?",[Nombre,Precio,id])
        if(resultado.affectedRows == 0) return res.status(404).json({message:"sabor de pan no encontrado"});
